...
def save_pdf_to_mongodb(input_path, collection, is_folder=True):
    if not is_folder and hasattr(input_path, "name"):
        input_path = input_path.name

    if is_folder:
        if not os.path.isdir(input_path):
            raise ValueError(f"Invalid folder path: {input_path}")
        pdf_files = [pdf for pdf in os.listdir(input_path) if pdf.endswith((".pdf", ".txt"))]
    else:
        pdf_files = [input_path]

    for pdf_file in pdf_files:
        file_path = os.path.join(input_path, pdf_file) if is_folder else pdf_file
        pdf_content = read_pdf(file_path)
        if pdf_content:
            pdf_hash = generate_hash(pdf_content)
            if collection.find_one({"hash": pdf_hash}):
                print(f"Duplicate detected: {file_path} already exists in MongoDB. Skipping...")
            else:
                pdf_data = {
                ...
                print(f"Saved {file_path} to MongoDB.")
...
def build_vector_store(chunks, embeddings, collection):
    print("Building vector store...")
    try:
        for chunk in chunks:
            chunk_hash = generate_hash(chunk.page_content)
            existing_document = collection.find_one({"chunk_hash": chunk_hash})
        ...
        print(f"Vector store built successfully for collection: {collection}")
...
def load_vector_store(collection):
    try:
...
def get_chain(user_input, folder_path, file_input):
    ...
    save_pdf_to_mongodb(folder_path, collection_pdf)
    save_pdf_to_mongodb(file_input, collection_pdf_user, is_folder=False)
    ...
    build_vector_store(chunks, embeddings, collection_embedding)
    build_vector_store(chunks_user, embeddings_user, collection_embedding_user)
    print("Done build_vector_store")

    vector_store = load_vector_store(collection_embedding)
    vector_store_user = load_vector_store(collection_embedding_user)
...
# collection_pdf.delete_many({})
# collection_pdf_user.delete_many({})
# collection_embedding_user.delete_many({})
"""
